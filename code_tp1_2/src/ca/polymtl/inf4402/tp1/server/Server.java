...
public class Server implements ServerInterface {
  ...
  private void createInMemory(String filename, byte[] data, String clientId) throws NoSuchAlgorithmException {
  ...
  /**
   * 
   * @return Si le checksum est different, retourne le nouveau data, sinon null;
   */
  @Override
  public byte[] lock(String filename, String clientId, String checksum) {
    ...
    if (fileToRetrieve.isLocked()) {
      throw new IllegalArgumentException(filename + " est déjà verrouillé par " + fileToRetrieve.getClientId());
    }
  ...
  public String list() {
    ...
    for (FilePoly file : fileMap.values()) {
      String clientId = file.getClientId() != null ? " verrouillé par " + file.getClientId() : " non vérrouillé";
